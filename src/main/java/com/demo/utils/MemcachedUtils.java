...
public class MemcachedUtils {
	...
	static {
		if (cachedClient == null) {
			cachedClient = (MemCachedClient) SpringContextUtils.getBeanById("memcachedClient");
	...
	private static class MemcachedLog {
		...
		private final static String LINUX_MEMCACHED_LOG = "/home/cotton/logs/memcached.log";
		...
		public static void writeLog(String content) {
			try {
				logWrite.write("[" + PID + "] " + "- [" + DateUtils.getSysDateTimeStr() + "]\r\n" + content);
				logWrite.newLine();
				logWrite.flush();
			} catch (IOException e) {
	...
	public static void main(String[] args) {
		/*
		String[] servers = { "192.168.1.22:11211"};
		...
		*/
		...
		for (int i = 0; i <= 2; i++) { // 插入*万条数据
			MemcachedUtils.add("abc" + i, "hello" + i);// 添加数据
			System.out.println("get value : " + MemcachedUtils.get("abc" + i));// 获取数据
		}
